...
void LC_MakerCamSVG::writeEllipse(RS_Ellipse *ellipse)
{
    ...
    if (convertEllipsesToBeziers)
    {
        ...
        if (ellipse->isEllipticArc())
        {
            ...
            RS_DEBUG->print("RS_MakerCamSVG::writeEllipse: Writing ellipse arc approximated by 'path' with %d cubic bézier segments (as discussed in https://www.spaceroots.org/documents/ellipse/elliptical-arc.pdf) ...",
                            segments);
