...
namespace
{
	...
	public:
		...
		std::tuple<double, double, double> operator()(double const& z) const
		{
			...
			// return               f(x),                           f'(x),  f''(x)
			return std::make_tuple(e->getEllipseLength(z) - distance, ra * d, k2ra * sz * cz / d);
		}
	...
	*/
		...
		double* dist = nullptr)
	{
		...
		if (trimEnd) { trimmed = trimAmount > 0 ? wholeLength - trimAmount : -trimAmount; }

		//solve equation of the distance by second order newton_raphson
		EllipseDistanceFunctor X(&e, trimmed);
		//using namespace boost::math::tools;
		//double const sol =halley_iterate<EllipseDistanceFunctor,double>(X,
		//													  guess,
		//													  x1,
...
double RS_Ellipse::getEllipseLength(double x1, double x2) const
{
	...
	if (x2 >= M_PI)
	{
		ret = (static_cast<int>((x2 + RS_TOLERANCE_ANGLE) / M_PI) - (static_cast<int>((x1 + RS_TOLERANCE_ANGLE) / M_PI))) * 2;
...
/**
	...
	using Newton-Raphson from boos2t
