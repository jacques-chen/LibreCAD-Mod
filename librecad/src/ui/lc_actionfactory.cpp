...
void LC_ActionFactory::fillActionContainer(QMap<QString, QAction *> &a_map, LC_ActionGroupManager *agm)
{
    ...
    action = new QAction(tr("&Copy"), agm->modify);
    action->setIcon(QIcon(":/icons/modify_copy.svg"));
    connect(action, SIGNAL(triggered()), action_handler, SLOT(slotModifyCopy()));
    action->setObjectName("ModifyCopy");
    action->setData("copy, co");
    a_map["ModifyCopy"] = action;

    action = new QAction(tr("&Move"), agm->modify);
    action->setIcon(QIcon(":/icons/modify_move.svg"));
