...
void RS_ActionDimLinear::reset()
{
    ...
    edata.reset(new RS_DimLinearData(RS_Vector(false), RS_Vector(false), 0.0, 0.0));
...
void RS_ActionDimLinear::preparePreview()
{
    ...
    RS_ConstructionLine cl( NULL, RS_ConstructionLineData( edata->extensionPoint2, edata->extensionPoint2 + dirV));
...
void RS_ActionDimLinear::mouseMoveEvent(QMouseEvent *e)
{
    ...
    switch (getStatus())
    {
        ...
        case SetExtPoint2:
            ...
            {
                deletePreview();
                preview->addEntity(new RS_Line{preview.get(), edata->extensionPoint1, mouse});
                drawPreview();
            }
            break;
        ...
        case SetDefPoint:
            ...
            {
                if (RS_Math::isBetween(mouse.x, (edata->extensionPoint1).x, (edata->extensionPoint2).x) == true &&
                    RS_Math::isBetween(mouse.y, (edata->extensionPoint1).y, (edata->extensionPoint2).y) == true)
                {
                    //setAngle(0.0);
                }
                else  if (RS_Math::isBetween(mouse.x, (edata->extensionPoint1).x, (edata->extensionPoint2).x) == true &&
                          RS_Math::isBetween(mouse.y, (edata->extensionPoint1).y, (edata->extensionPoint2).y) == false)
                {
                    setAngle(0.0);
                }
                else if (RS_Math::isBetween(mouse.x, (edata->extensionPoint1).x, (edata->extensionPoint2).x) == false &&
                         RS_Math::isBetween(mouse.y, (edata->extensionPoint1).y, (edata->extensionPoint2).y) == true)
                {
                    setAngle(M_PI_2);
                }
